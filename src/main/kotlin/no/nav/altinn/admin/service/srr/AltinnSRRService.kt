...
import no.altinn.schemas.services.register.srr._2015._06.AddRightRequest
import no.altinn.schemas.services.register.srr._2015._06.AddRightRequestList
import no.altinn.schemas.services.register.srr._2015._06.AddRightResponseList
import no.altinn.schemas.services.register.srr._2015._06.DeleteRightRequest
import no.altinn.schemas.services.register.srr._2015._06.DeleteRightRequestList
...
class AltinnSRRService(private val env: Environment, iRegisterSRRAgencyExternalBasicFactory: () -> IRegisterSRRAgencyExternalBasic) {
    ...
    fun getRightsForAllBusinesses(tjenesteKode: String, utgaveKode: String = "1"): RightsResponseWithList {
        try {
            ...
            val register = env.mock.srrGetResponse
                ?: iRegisterSRRAgencyExternalBasic.getRightsBasic(
    ...
    private fun createAddResponseMessage(sc: String, response: AddRightResponseList, type: RegisterSRRRightsType, reportee: String): RightsResponse {
        if (response.addRightResponse.size != 1) {
            ...
            return RightsResponse(
                "Failed",
                "Expected only one AddRightsResponse in list, size is " + response.addRightResponse.size
            )
        }
    ...
    private fun createDeleteResponseMessage(sc: String, response: DeleteRightResponseList, type: RegisterSRRRightsType, reportee: String): RightsResponse {
        if (response.deleteRightResponse.size != 1) {
            ...
            return RightsResponse(
                "Failed",
                "Expected only one DeleteRightsResponse in list, size is " + response.deleteRightResponse.size
            )
        }
